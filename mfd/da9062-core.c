...
#define CONFIG_ZYNQMP_IWG30M_H
#endif

#ifdef CONFIG_ZYNQMP_IWG30M_H
#include <linux/of_gpio.h>
#include <linux/gpio.h>
#define BSP_VERSION             "iW-PRFUZ-SC-01-R2.0-REL1.0-UD2.0-Petalinux22.2"

void iwg30m_zynqmp_board_info(struct da9062 *chip);
#endif
...
/* IWG30M: Function to print the Board configuration number using PMIC register and PL GPIOs */
#ifdef CONFIG_ZYNQMP_IWG30M_H
void iwg30m_zynqmp_board_info(struct da9062 *chip)
{
	int pcb_rev=2, bom_rev, y9, ac13, ac13_val=0, pmic_st, status_b, bom_ascii=71;
	...
	node = of_find_node_by_name(NULL,"som_rev");
	if(node){
		/* IWG30M: Read the Y9 GPIO from the dts file*/
		y9 = of_get_named_gpio(node, "pl-y9", 0);
		if (gpio_is_valid(y9)){
			gpio_request(y9, "pl-io-y9");
			pcb_rev = gpio_get_value(y9);
			if (pcb_rev < 0)
				pcb_rev = 2;
			else
				pcb_rev += 2;
		...
		/* IWG30M: Read the AC13 GPIO from the dts file*/
		ac13 = of_get_named_gpio(node, "pl-ac13", 0);
		if (gpio_is_valid(ac13)){
			gpio_request(ac13, "pl-io-ac13");
			ac13_val = gpio_get_value(ac13);
		}
	}

	/* IWG30M: Read the PMIC STATUS_B register*/
	pmic_st = regmap_read(chip->regmap, DA9062AA_STATUS_B, &status_b);
	status_b &= 0xF;

	if(ac13_val == 1)
		bom_rev = bom_ascii + status_b;
	else
		bom_rev = status_b;

	if(bom_rev <= 0xF)
		printk ("\tSOM Version     : iW-PRFUZ-AP-01-R%d.%X\n",pcb_rev,bom_rev);
	else
		printk ("\tSOM Version     : iW-PRFUZ-AP-01-R%d.%x\n",pcb_rev,bom_rev);
	printk ("\n");
}
...
/* IWG30M: RTC: Disable device id read to avoid RTC driver probe issue with Device-id mismatch*/
#ifndef CONFIG_ZYNQMP_IWG30M_H
...
#ifdef CONFIG_ZYNQMP_IWG30M_H
	/*IWG30M: print the BSP and SOM version*/
	iwg30m_zynqmp_board_info(chip);
#endif
